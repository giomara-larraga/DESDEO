...
class AuthConfig(BaseModel):
    ...
    # openssl rand -hex 32
    authjwt_secret_key: ClassVar[StrictStr] = (
        "36b96a23d24cebdeadce6d98fa53356111e6f3e85b8144d7273dcba230b9eb18"
    )
...
class DBConfig(BaseModel):
    ...
    db_database: ClassVar[StrictStr] = (
        os.getenv("POSTGRES_DB") or "postgres"
    )  # postgres
    ...
    db_pool: ClassVar[StrictBool] = (os.getenv("POSTGRES_POOL") or True) in (
        True,
        "true",
        "1",
        "t",
...
class WebUIConfig(BaseModel):
    ...
    cors_origins: ClassVar[list] = json.loads(
        os.getenv(
            "CORS_ORIGINS",
            '["http://localhost", "http://localhost:8080", "http://localhost:5173", "http://127.0.0.1:5173"]',
        )
    )
