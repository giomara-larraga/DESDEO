...
router = APIRouter(prefix="/reference_point_method")


class InitRequest(BaseModel):
    """The request to initialize the RPM."""
...
class RPMResponse(BaseModel):
    """The response from most RPM endpoints."""
    ...
    current_solutions: list[list[float]] = Field(
        description="The solutions from the current interation of RPM."
    )
    ...
    current_multipliers: list[list[float | None]] | None = Field(
        description="The solutions from the current interation of RPM."
    )
    saved_multipliers: list[list[float | None]] | None = Field(
        description="The best candidate solutions saved by the decision maker."
    )
    all_multipliers: list[list[float | None]] | None = Field(
        description="All solutions generated by RPM in all iterations."
    )


class RPMIterateRequest(BaseModel):
    """The request to iterate the RPM algorithm."""

    problem_id: int = Field(description="The ID of the problem to be solved.")
    method_id: int = Field(description="The ID of the method being used.")
    kkt_multipliers: bool = Field(description="If the multipliers are needed or not")
    preference: list[float] = Field(
        description=("The preference as a reference point.")
    )
...
class FakeRPMResponse(BaseModel):
...
@router.post("/iterate")
def iterate(
    request: RPMIterateRequest,
    user: Annotated[User, Depends(get_current_user)],
    db: Annotated[Session, Depends(get_db)],
) -> RPMResponse | FakeRPMResponse:
    """Iterate the RPM algorithm.

    Args:
        request: The request body for a RPM iteration.
        user (Annotated[User, Depends(get_current_user)]): The current user.
        db (Annotated[Session, Depends(get_db)]): The database session.
    ...
    return RPMResponse(
        ...
        saved_solutions=[sol.objectives for sol in solutions if sol.saved],
        all_solutions=[sol.objectives for sol in solutions],
        current_multipliers=[sol.multipliers for sol in solutions if sol.current],
...
def save(
    ...
    problem_id = request.problem_id
    method_id = get_rpm_method_id(db)
    ...
    return RPMResponse(
        ...
        current_multipliers=[
            sol.multipliers for sol in previous_solutions if sol.current
        ],
        saved_multipliers=[sol.multipliers for sol in previous_solutions if sol.saved],
        all_multipliers=[sol.multipliers for sol in previous_solutions],
    )
...
def choose(
    ...
    return FakeRPMResponse(message="Solution chosen.")
...
def get_rpm_method_id(db: Session) -> int:
    """Queries the database to find the id for RPM method.
    ...
    Returns:
        The method id
    """
    rpm_method = db.query(Method).filter(Method.kind == Methods.RPM).first()
    return rpm_method.id
...
def save_results_to_db(
    db: Session,
    user_id: int,
    request: InitRequest | RPMIterateRequest,
    results: list,
