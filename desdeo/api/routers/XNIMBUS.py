...
from desdeo.mcdm.x_nimbus import (
    generate_starting_point,
    solve_intermediate_solutions,
    solve_sub_problems,
)
from desdeo.problem.schema import Problem
from desdeo.api.routers.NIMBUS import calculate_bounds, flatten

router = APIRouter(prefix="/xnimbus")
...
class NIMBUSIterateRequest(BaseModel):
    ...
    num_solutions: int | None = Field(
        description="The number of solutions to be generated in the iteration.",
        default=1,
    )
...
def iterate(
    ...
    return NIMBUSResponse(
        ...
        saved_multipliers=[sol.multipliers for sol in solutions if sol.saved],
        all_multipliers=[sol.multipliers for sol in solutions if sol.saved],
    )
...
def intermediate(
    ...
    results = solve_intermediate_solutions(
        problem=problem,
        solution_1=dict(
            zip(problem.objectives, request.reference_solution_1, strict=True)
        ),
        solution_2=dict(
            zip(problem.objectives, request.reference_solution_2, strict=True)
        ),
        num_desired=request.num_solutions,
    )
...
def save_results_to_db(
    ...
    if type(request) is InitRequest:
        ...
        pref = Preference(
            ...
            kind=(
                "XNIMBUS"
                if type(type(request) is NIMBUSIterateRequest)
                else "XNIMBUS_intermediate"
            ),
