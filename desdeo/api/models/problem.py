...
class SiteSelectionMetaData(SQLModel, table=True):
    ...
    sites_json: str = Field(
        description="JSON array: [{name, node, lat, lon}, ...] one per site variable"
    )
    nodes_json: str = Field(
        description="JSON array: [{name, lat, lon, size}, ...] one per map node"
    )
    travel_time_matrix_json: str = Field(
        description="JSON: 2D list[list[float]], shape [n_nodes, n_nodes]"
    )
    ...
    coverage_threshold: float = Field(
        default=15.0, description="Threshold for coverage edges (e.g., minutes, km)"
    )

    metadata_instance: "ProblemMetaDataDB" = Relationship(
        back_populates="site_selection_metadata"
    )
