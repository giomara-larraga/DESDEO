...
class BaseSelector(Subscriber):
    ...
    def __init__(
        ...
        self.maximization_mult = {
            x.symbol: -1 if x.maximize else 1 for x in problem.objectives
        }

        if problem.scalarization_funcs is None:
            self.target_symbols = [f"{x.symbol}_min" for x in problem.objectives]
            try:
                ideal, nadir = get_corrected_ideal_and_nadir(
                    problem
                )  # This is for the minimized problem
...
class BaseDecompositionSelector(BaseSelector):
    ...
    def __init__(
        ...
        if self.reference_vector_options.reference_point:
            ...
            corrected_sols = np.array(
                [
                    np.array(self.reference_vector_options.preferred_solutions[x])
                    * self.maximization_mult[x]
                    for x in self.objective_symbols
                ]
            ).T
            ...
            corrected_sols = np.array(
                [
                    np.array(self.reference_vector_options.non_preferred_solutions[x])
                    * self.maximization_mult[x]
                    for x in self.objective_symbols
                ]
            ).T
            self.interactive_adapt_2(
                ...
                ord=(
                    2 if self.reference_vector_options.vector_type == "spherical" else 1
                ),
            )
        elif self.reference_vector_options.preferred_ranges:
            corrected_ranges = np.array(
                [
                    np.array(self.reference_vector_options.preferred_ranges[x])
                    * self.maximization_mult[x]
                    for x in self.objective_symbols
                ]
            ).T
    ...
    def _create_simplex(self):
        ...
        if (
            not self.invert_reference_vectors
        ):  # todo, this currently only exists for nsga3
    ...
    def interactive_adapt_2(
        self, z: np.ndarray, predefined_distance: float, ord: int
    ) -> None:
...
class ParameterAdaptationStrategy(StrEnum):
    ...
    FUNCTION_EVALUATION_BASED = "FUNCTION_EVALUATION_BASED"  # Based on the current function evaluation and the maximum function evaluation.
    ...
    for i in range(ref_dirs.shape[0]):
        for j in range(solutions.shape[0]):
            ...
            component = (
                ref_dirs[i]
                - solutions[j]
                - np.dot(ref_dirs[i] - solutions[j], unit_vector) * unit_vector
            )
...
class NSGA3Selector(BaseDecompositionSelector):
    ...
    def associate_to_niches(
        ...
        dist_matrix = jitted_calc_perpendicular_distance(
            N, ref_dirs, self.invert_reference_vectors
        )
...
class IBEASelector(BaseSelector):
    ...
    def __init__(
        ...
        super().__init__(
            problem=problem, verbosity=verbosity, publisher=publisher, seed=seed
        )
        self.selection: list[int] | None = None
        self.selected_individuals: SolutionType | None = None
        self.selected_targets: pl.DataFrame | None = None
