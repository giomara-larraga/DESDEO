...
def pbea_options() -> EMOOptions:
    """Get default Preference-Based Evolutionary Algorithm (PBEA) options.

    PBEA follows Thiele, Miettinen, Korhonen, and Molina (2009) and uses the
    Template 2 IBEA loop. With ``preference=None`` it reduces to ordinary
    epsilon-IBEA, which corresponds to Step 0 of their interactive procedure.
    Supply a ``ReferencePointOptions(method="PBEA", ...)`` to activate the
