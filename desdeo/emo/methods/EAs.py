...
def nsga3_archive(
    *,
    ...
    It uses an archive generator that can take solutions from previous generation.
    ...
    Args:
        problem (Problem): The problem to be solved.
        solutions (DataFrame): The solutions from previous generation to be used by the archive generator.
        seed (int, optional): The seed for the random number generator. Defaults to 0.
        n_generations (int, optional): The number of generations to run the algorithm. Defaults to 100.
        max_evaluations (int, optional): The maximum number of evaluations to run the algorithm. If None, the algorithm
            will run for n_generations. Defaults to None. If both n_generations and max_evaluations are provided, the
...
def ibea(
    *,
    ...
    scalar_selector = TournamentSelection(
        publisher=publisher, verbosity=0, winner_size=population_size, seed=seed
    )
