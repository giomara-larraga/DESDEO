<script lang="ts">
	...
	let {
		...
		handle_remove_saved?: (solution: Solution, rowIndex?: number) => void;
	...
	let effectiveExpandedIndexes = $derived.by(() =>
		expandedRowsData.length > 0 && expandedRowIndexes.length === expandedRowsData.length
			? expandedRowIndexes
			: expandedRowsData.map((_, idx) => idx + 1)
	);
	...
	let displayName = $derived((idx: number | null, iterationNumber?: number) => {
		...
		switch (selected_type_solutions) {
			...
			default: // 'current' and 'all_group'
				if (totalSolutionsCount > 1 && idx !== null) {
	...
	const columns: ColumnDef<Solution>[] = $derived.by(() => {
		return [
			...
			{
				accessorKey: 'saved',
				size: COLUMN_WIDTHS.saved,
				minSize: COLUMN_WIDTHS.saved,
				maxSize: COLUMN_WIDTHS.saved,
			...
			{
				accessorKey: 'edit',
				size: COLUMN_WIDTHS.edit,
				minSize: COLUMN_WIDTHS.edit,
				maxSize: COLUMN_WIDTHS.edit,
				header: ({ column }) => renderSnippet(ColumnHeader, { column, title: '' }),
			...
			...(selected_type_solutions !== 'current'
				? [
						{
							id: 'iteration_number',
							size: COLUMN_WIDTHS.iteration,
							minSize: COLUMN_WIDTHS.iteration,
	...
	function onTableRowClick(index: number, rowId: string) {
				...
				<Button
					...
					onclick={(e) => {
						...
						if (handle_remove_saved) {
							handle_remove_saved(solution, rowIndex);
						}
					}}
				>
...
{#snippet ExpandCell({
	...
	<div class="flex w-10 justify-center">
		{#if canExpand}
			...
			>
				<ChevronRightIcon class={`h-4 w-4 transition-transform ${isExpanded ? 'rotate-90' : ''}`} />
			</Button>
		{:else}
			...
			<Table.Root>
				...
				<Table.Body>
					{#each table.getRowModel().rows as row (row.id)}
						<Table.Row
							onclick={!isFrozen ? () => onTableRowClick(row.index, row.id) : undefined}
							...
							{#if expandedRowsData.length > 0}
								{#each expandedRowsData as expandedSolution, expandedIndex (`${row.id}-expanded-${expandedIndex}`)}
									...
									>
										...
										<Table.Cell style={`width: ${COLUMN_WIDTHS.edit}px;`}>
											{@render EditCell({ solution: expandedSolution })}
										</Table.Cell>
										{#if selected_type_solutions !== 'current'}
											<Table.Cell style={`width: ${COLUMN_WIDTHS.iteration}px;`}>
												{@render IterationCell({ solution: expandedSolution })}
											</Table.Cell>
										{/if}
										{#each problem.objectives as objective, objectiveIndex (`${objective.symbol}-${expandedIndex}`)}
											<Table.Cell style={`width: ${COLUMN_WIDTHS.objective}px;`}>
												{@render ObjectiveCell({
													value: expandedSolution.objective_values?.[objective.symbol],
													accuracy: displayAccuracy[objectiveIndex]
												})}
											</Table.Cell>
										{/each}
									</Table.Row>
