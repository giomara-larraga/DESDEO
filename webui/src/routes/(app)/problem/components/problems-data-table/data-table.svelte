<script lang="ts">
	...
	const columns: ColumnDef<Problem>[] = [
		...
		},
		{
			...
			cell: ({ row }) => {
				return renderSnippet(DefinedByCell, {
					value: row.original.definedby
				});
			},
			filterFn: (row, id, value) => {
				return value.includes(row.getValue(id));
...
{#snippet RowActions({ row }: { row: Row<Problem> })}
...
{#snippet ColumnHeader({
	...
	class: className,
	...restProps
}: { column: Column<Problem>; title: string } & HTMLAttributes<HTMLDivElement>)}
