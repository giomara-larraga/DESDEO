...
type RPMSolveResponse = components['schemas']['RPMSolveResponse'];
type RPMInitializationResponse = components['schemas']['RPMInitializationResponse'];
type Response = RPMSolveResponse | RPMInitializationResponse | null;
...
export async function callRPMAPI<T = Response>(type: string, data: Record<string, any>): Promise<{
    success: boolean;
    data?: T;
    error?: string;
}> {
    try {
        const response = await fetch(`/interactive_methods/RPM/?type=${type}`, {
            method: 'POST',
            headers: {
