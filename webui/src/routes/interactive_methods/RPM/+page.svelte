<script lang="ts">
	/**
	 * +page.svelte (Reference point method)
	 *
	 * @author Giomara Larraga (Base structure)<glarragw@jyu.fi>
	 * @created April 2026
	 *
	 * @description
	 * This page implements the Reference Point method for interactive multiobjective optimization in DESDEO.
	...
	/*async function initialize_nimbus_state(problem_id: number) {
		...
		if (result) {
			...
			update_preferences_from_state(current_state);
			current_num_iteration_solutions = current_state.current_solutions.length;
			if (current_state.response_type === 'rpm.finalize') {
				mode = 'final';
			}
		}
	}*/
